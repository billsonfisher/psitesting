...
Class PMS_Payment_Gateway_PayPal_Pro extends PMS_Payment_Gateway {
    ...
    public function process_sign_up() {
        ...
        if ($api_credentials){
            ...
            $api_signature = $api_credentials['signature'];
        }
        ...
        if ( is_wp_error($resp) ) {  // PayPal post failed
            ...
            $error_message = __( 'Something went wrong. Please try again.', 'paid-member-subscriptions' );
            $error_message .=  $resp->get_error_message();

        }

        elseif ( 200 == $resp['response']['code'] && 'OK' == $resp['response']['message'] ) {
            ...
            if ((strtolower($paypal_data['ACK']) == 'failure') || (strtolower($paypal_data['ACK']) == 'failurewithwarning')) {
                ...
                $error_message =  __('PayPal subscription creation failed. Please try again.', 'paid-member-subscriptions') . '</br>';
                $error_message .= __('Error message:', 'paid-member-subscriptions') . ' ' . $paypal_data['L_LONGMESSAGE0'] . '</br>';
                $error_message .= __('Error code:', 'paid-member-subscriptions') . ' ' . $paypal_data['L_ERRORCODE0'];
                ...
                // Update member data only if the payment is a web_accept,
                // Recurring payments will be handled by the web_hook found in class-payment-gateway-paypal-express.php
                if ( isset($this->form_location) && $payment_type == 'web_accept_paypal_pro' ) {
        ...
        if( isset($error_message) )
            pms_errors()->add( 'billing_cc', $error_message );
