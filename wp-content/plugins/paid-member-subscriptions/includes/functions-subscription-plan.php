...
if ( ! defined( 'ABSPATH' ) ) exit;
    ...
    if( ! function_exists( 'pms_renewal_option_field' ) ) {
        ...
        function pms_renewal_option_field( $output, $include, $exclude_id_group, $member, $pms_settings ) {
            ...
            if( ( isset( $pms_settings['payments']['recurring'] ) && $pms_settings['payments']['recurring'] == 1 ) || $subscription_plan_renew ) {
            ...
            if( ! empty( $pms_settings['payments']['recurring'] ) )
                $output .= '<input type="hidden" name="pms_default_recurring" value="' . esc_attr( $pms_settings['payments']['recurring'] ) . '" />';
